...
#ifndef KATAGO_ATTAX_BOARD_H
#define KATAGO_ATTAX_BOARD_H
...
static constexpr int NUM_BOARD_COLORS = 3;
...
namespace Location
{
    Loc getLoc(Coord x, Coord y, Coord x_size);
    int getX(Loc loc, Coord x_size);
    int getY(Loc loc, Coord x_size);
...
struct Board
{
    ...
    static const Coord MAX_LEN = 7;  //Maximum edge length allowed for the board
    static const Coord MAX_PLAY_SIZE = MAX_LEN * MAX_LEN;  //Maximum number of playable spaces
    static const Coord MAX_ARR_SIZE = (MAX_LEN+1)*(MAX_LEN+1); //Maximum size of arrays needed

    //Location used to indicate an invalid spot on the board.
    static const Loc NULL_LOC = MAX_ARR_SIZE;
    //Location used to indicate a pass move is desired.
    static const Loc PASS_LOC = MAX_ARR_SIZE - 1;
